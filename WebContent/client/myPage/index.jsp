...
function myGame(){
   $.ajax({
      ...
      success:function(result){
		...
		$("p[name='gameNum']").append("보유 게임 수 : "+num+" 개");
      }
   });
}
...
function getProfile() {
   $.ajax({
      ...
      success : function(result) {
         $("h1[name='id']").text("아이디 : "+result.id);
         $("h1[name='name']").text("이름 : "+result.name);
         $("h1[name='nick']").text("닉네임 : "+result.nick);
         $("h1[name='email']").text("이메일 : "+result.email);
      }
   });
}
</script>
